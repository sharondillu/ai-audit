  ...
  try {
    ...
    const changesHtml = changes.map((c) => `
      <tr>
        ...
        <td style="padding:10px 16px;border-bottom:1px solid #1e293b;color:#f87171;text-decoration:line-through;">
          $${c.oldPrice}/mo
        </td>
        <td style="padding:10px 16px;border-bottom:1px solid #1e293b;color:#34d399;">
          $${c.newPrice}/mo
        </td>
      </tr>
    `).join("");
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: `
          <div style="max-width:600px;margin:40px auto;background:#1e293b;border-radius:16px;overflow:hidden;font-family:system-ui,sans-serif;">
